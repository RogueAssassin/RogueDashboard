...
const ICON_REMOTE_OVERRIDES = {
  ...
  roguedashboard: "/branding/roguedashboard.svg?v=1.2.1",
};
...
async function load() {
  try {
    ...
    app.innerHTML = `<main class="center-stage"><section class="error-card"><div class="brand-mark"><img data-rgd-brand-image src="/branding/roguedashboard-dark.svg?v=1.2.1" alt="RogueDashboard"></div><h1>Dashboard unavailable</h1><p>${escapeHtml(error.message)}</p><button class="button primary" id="retry">Try again</button></section></main>`;
  ...
  app.innerHTML = `
    <main class="setup-shell">
      ...
      <section class="setup-card">
        <header class="setup-brand"><div class="brand-mark"><img data-rgd-brand-image src="/branding/roguedashboard-dark.svg?v=1.2.1" alt="RogueDashboard"></div><div><strong>RogueDashboard</strong><span>Service dashboard</span></div></header>
  ...
  app.innerHTML = `
    <div class="app-shell theme-${escapeHtml(dashboard.meta.theme)} background-${escapeHtml(dashboard.meta.backgroundMode)} density-${escapeHtml(dashboard.meta.density)} ${state.editor ? "editing" : ""}" id="shell">
      ...
      <main class="dashboard ${dashboard.meta.fullWidth ? "full-width" : ""}">
        <header class="topbar">
          <div class="brand-block"><div class="brand-mark small"><img data-rgd-brand-image src="/branding/roguedashboard-dark.svg?v=1.2.1" alt=""></div><div><h1>${escapeHtml(dashboard.meta.title)}</h1><p>${escapeHtml(dashboard.meta.subtitle)}</p></div></div>
